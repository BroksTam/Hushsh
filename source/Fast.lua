...
send(msg_chat_id,msg_id,"مرحبآ بك عزيزي المبرمج قيصر نورت الكروب - 🔥","md",true)
end
end
if msg.content.Fastbots == "messageChatJoinByLink" or msg.content.Fastbots == "messageChatAddMembers" then
...
if text and text:match("^تشاك (.*)$") then
local TextName = text:match("^تشاك(.*)$")
...
local Name_dev = bot.getUser(Sudo_Id).first_name
local UName_dev = bot.getUser(Sudo_Id).username
local CmdStart = '*\n- اهلا بك عزيزي انا بوت اسمي '..(Redis:get(Fast.."Name:Bot") or "⦗تشاك⦘")..
