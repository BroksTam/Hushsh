...
return bot.answerCallbackQuery(data.id,"\n حبي انته لوتي غير انته صاحب السؤال ؟", true)
end
...
data = {
{
{text = 'انظم للعب !', data = Game_king..'/add7azer'}, 
},
{
...
if Text and Text:match('(%d+)/normal_zk') then
  ...
  if tonumber(UserId) == tonumber(IdUser) then
    ...
    edit(ChatId, Msg_id, "*↯ ︙ارسل الكلمه لزخرفتها عربي او انكليزي*", "md",false)
  end
  end
if Text and Text:match('(%d+)/inline_zk') then
  ...
  if tonumber(UserId) == tonumber(IdUser) then
    local reply_markup = bot.replyMarkup{
      ...
      data = {
      {
      {text = 'زخرفه انكليزي .', data = IdUser..'/zeng'},
      },
      {
        {text = 'زخرفه عربي .', data = IdUser..'/zar'},
        },
      }
      }
...
caption = ' المطي هاذ '..tag..' هو الي قتلها',
parse_mode = "Markdown"
}
local keyboard = {}
...
local Text = "*↯ ︙ حسنا ارسل لي يوزر بوتك الان !*"
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
...
local Text = "* ↯ ︙ حسنا ارسل لي رابط ڪروبك الان *"
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
...
return edit(ChatId,Msg_id,"كولولولولويششش\nاليوم عقدنا قران :\n\nالزوج "..zwg_tag.." 🤵🏻\n   💗\nالزوجة "..zwga_tag.." 👰🏻‍♀️\nالمهر : "..convert_mony.." جنيه بعد الضريبة 15%\nعلمود تشوفون وثيقة زواجكم اكتبو : زواجي", 'md', false)
else
return edit(ChatId,Msg_id,"انتهى الطلب وين جنتي من طلب ايدج ؟", 'md', false)
end
end
end
...
return bot.answerCallbackQuery(data.id, "شنو رأيك نزوجك بدالهم ؟", true)
else
Redis:del(Fast.."zwag_request:"..UserId[1])
Redis:del(Fast.."zwag_request:"..UserId[2])
return edit(ChatId,Msg_id,"خلي القطار يفوتج يمطي !", 'md', false)
end
end
----
...
if tonumber(data.sender_user_id) == tonumber(UserId) then
local toptop = " ↯ ︙اهلا بيك في قائمه التوب\nللمزيد من التفاصيل  [@IOUTB]\n"
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
{text = '- النوادي', data = data.sender_user_id..'/topnade'},{text = '- الحراميه', data = data.sender_user_id..'/topzrf'},{text = '- الفلوس', data = data.sender_user_id..'/topmon'},{text = '- الزواج', data = data.sender_user_id..'/zoztee'},
},
{
...
return bot.editMessageText(chat_id,msg_id, "↯ ︙ماتكدر تشتري فلوسك ماتكفي","md",true)
end
...
return bot.editMessageText(chat_id,msg_id, "↯ ︙ ماتكدر تشتري فلوسك متكفي","md",true)
end
...
data = {
{
{text = '✦ َ𝐒𝐎𝐔𝐑𝐂𝐄 𝐂𝐀𝐄𝐒𝐀𝐑✦', url = 't.me/IOUTB'}, 
},
}
}
...
local zwg_id = anubis[2]
if tonumber(zwga_id) == tonumber(IdUser) then
return edit(ChatId, Msg_id, "لعد شوكت حتزوج يربج ؟","md",false)
end
end
if Text and Text:match('(%d+)/zeng') then
local UserId = Text:match('(%d+)/zeng')
...
local zwg_id = anubis[2]
if tonumber(zwga_id) == tonumber(IdUser) then
return edit(ChatId, Msg_id, "     ؟؟ادري لعد اجيبلك منو حته تتزوج","md",false)
end
end
------
...
type = "photo",
media = ph,
caption = '*  *[آهــلٓآ بَٰــڪ فٍُــيَ س๋͜‏ــۄرس๋͜‏ قيصر 🕷️🔥](t.me/IOUTB)*\n\n*[ آقٍُـۄيَ مــصَٰـآنــعَِٰ س๋͜‏ــۄࢪس๋͜‏ــآټِٖ عَِٰــلٓيَ آلٓټِٖـلٓيَـج ـࢪآم](t.me/IOUTB) *\n\n*[ س๋͜‏ؤࢪس๋͜‏ قيصر عَِٰلٓيَ منعَِٰ ټِٖصَٰفٍُيَه آلٓمج ـمۄعَِٰآټِٖ ۄح ـمآيَټِٖهآ](t.me/IOUTB)*\n\n*[ لٓــديَــنــآ ټِٖـنصَٰيَــبَٰ بَٰـۄټِٖــآټِٖ مــيَـۄزڪ آيَــضَِٖـآ](t.me/IOUTB)*\n\n*[لٓديَــنآ ټِٖنـصَٰيَــبَٰ ح ـس๋͜‏ــآبَٰــآټِٖ ټِٖــلٓيَــثُِٖــۄن](t.me/IOUTB) *\n\n*[آلٓس๋͜‏ۄࢪس๋͜‏ مزۄد بَٰج ـميَعَِٰ آلآلٓعَِٰآبَٰ ۄآلٓمميَزآټِٖ آلٓمټِٖطِٖۄࢪه](t.me/IOUTB) *\n\n*[ټِٖآبَٰعَِٰ قٍُنآه آلٓټِٖح ـديثُِٖآټِٖ ۄآلٓټِٖطِٖۄيَࢪآټِٖ لٓيَصَٰلٓڪ ڪلٓ ج ـديَد لٓديَنآ](t.me/c_a_s_e_r_h) *\n\n*',
parse_mode = "Markdown"                                                                                                                                                               
}     
keyboard = {} 
keyboard.inline_keyboard = {
...
local TextHelp = Reply_Status(replyy,"↯ ︙تم العاء الحظر من الكروب ").Reply
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
end
...
data = {
{
{text = '✦ َ𝐒𝐎𝐔𝐑𝐂𝐄 𝐂𝐀𝐄𝐒𝐀𝐑✦', url = 't.me/'..chsource..''}, 
},
{
...
data = {
{
{text = '⌁ 𝐒𝐎𝐔𝐑𝐂𝐄 𝐂𝐀𝐄𝐒𝐀𝐑⚡', url = 't.me/IOUTB'}, 
},
}
}
