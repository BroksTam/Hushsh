...
data = {
{
{text = GetSetieng(ChatId).lock_links, data = '&'},{text = 'الروابط : ', data =IdUser..'/'.. 'Stats_link'},
},
{
...
if Text and Text:match('(%d+)/Stats_link') then
local UserId = Text:match('(%d+)/Stats_link')
if tonumber(IdUser) == tonumber(UserId) then
local reply_markup = bot.replyMarkup{type = 'inline',data = {
{
{text = 'بالطرد', data =UserId..'/'.. 'lock_linkkick'},{text = 'بالكتم ', data =UserId..'/'.. 'lock_linkktm'},
},
{
{text = '⦗ رجوع ⦘', data =UserId..'/'.. 'NoNextSeting'},
},
